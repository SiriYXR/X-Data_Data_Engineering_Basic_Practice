...
def get_simple_feature(label_field):
    ...
    feature['simple_User_id_Coupon_id_receive_before_cnt'] = feature['simple_User_id_Coupon_id_receive_before_cnt'].map(int)
...
def get_history_field_merchant_feature(label_field, history_field):
    ...
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='User_id', aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
    ...
    feature[prefixs + 'receive_and_consume_differ_User_differ_User_rate'] = list(
        map(lambda x: x / differ_User if differ_User > 0 else 0, feature[prefixs + 'receive_and_consume_differ_User_cnt']))
    ...
    feature[prefixs + 'receive_and_consume_differ_Coupon_receive_differ_Coupon_rate']=list(
        map(lambda x: x / differ_Coupon if differ_Coupon > 0 else 0, feature[prefixs + 'receive_and_consume_differ_Coupon_cnt']))
...
def get_history_field_user_feature(label_field, history_field):
    ...
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Merchant_id', aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
    ...
    feature[prefixs + 'receive_and_consume_differ_Merchant_receive_differ_rate']=list(map(lambda x,y:x/y if y>0 else 0,
...
def model_xgb(train, test):
    ...
    model = xgb.train(params, dtrain, num_boost_round=7314, evals=watchlist)
    # 预测
...
if __name__ == '__main__':
    ...
    feat_importance.to_csv(r'./output_files/feat_importance.csv')
