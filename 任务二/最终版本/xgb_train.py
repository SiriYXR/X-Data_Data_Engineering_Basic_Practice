...
def model_xgb(train, test):
    ...
    model = xgb.train(params, dtrain, num_boost_round=1200, evals=watchlist, early_stopping_rounds=50)
    # 预测
...
def model_xgb2(train, test):
    ...
    # 训练
    watchlist = [(dtrain, 'train')]
    model = xgb.train(params, dtrain, num_boost_round=250, evals=watchlist, early_stopping_rounds=50)
    # 预测
    predict = model.predict(dtest)
    # 处理结果
    predict = pd.DataFrame(predict, columns=['prob'])
...
if __name__ == '__main__':
    ...
    result, feat_importance = model_xgb2(big_train, test)
    # 保存
