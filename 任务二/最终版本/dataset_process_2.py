...
def prepare(dataset):
    ...
    int(str(x).split(':')[0]))
    # # 优惠卷的减免类型（满0~50/50~200/200~500 减）
    # data['min_cost_of_manjian_0_50'] = list(
    #     map(lambda x: 1 if x >= 0 and x < 50 else 0, data['min_cost_of_manjian']))
    # data['min_cost_of_manjian_50_200'] = list(
    #     map(lambda x: 1 if x >= 50 and x < 200 else 0, data['min_cost_of_manjian']))
    # data['min_cost_of_manjian_200_500'] = list(
    #     map(lambda x: 1 if x >= 200 else 0, data['min_cost_of_manjian']))
...
def get_week_feature(label_field):
    ...
    - 根据date_received列得到领卷日是周几,新增一列week存储,并将其one-hot离散为week_0,week_1,week_2,week_3,week_4,week_5,week_6;
    - 根据week列得到领卷日是否为休息日,新增一列is_weekend存储;
    - 领取优惠卷是一月的第几天
    ...
    print('-- 领卷日是否为休息日')
    feature['is_weekend'] = feature['week'].map(lambda x: 1 if x == 5 or x == 6 else 0)  # 判断领卷日是否为休息日
...
def get_simple_feature(label_field):
    ...
    # 用户是否是第一次 / 最后一次领取优惠卷
    print('-- 用户是否是第一次 / 最后一次领取优惠卷')
    keys = ['User_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    feature[prefixs+'first_time_recive']=list(map(lambda x:1 if x==-1 else 0, feature[prefixs+'receive_before_gap']))
    feature[prefixs + 'last_time_recive'] = list(
        map(lambda x: 1 if x == -1 else 0, feature[prefixs + 'receive_last_gap']))
    feature.fillna(0, downcast='infer', inplace=True)

    # 用户是否是第一次 / 最后一次领取特定优惠卷
    print('-- 用户是否是第一次 / 最后一次领取特定优惠卷')
    keys = ['User_id','Coupon_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    feature[prefixs + 'first_time_recive'] = list(
        map(lambda x: 1 if x == -1 else 0, feature[prefixs + 'receive_before_gap']))
    feature[prefixs + 'last_time_recive'] = list(
        map(lambda x: 1 if x == -1 else 0, feature[prefixs + 'receive_last_gap']))
    feature.fillna(0, downcast='infer', inplace=True)

    # 用户领取特定商家的优惠卷数目
    print('-- 用户领取特定商家的优惠卷数目')
    ...
    # 用户领取的不同折扣率优惠卷种类数目
    print('-- 用户领取的不同折扣率优惠卷种类数目')
    keys = ['User_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data, index=keys, values='discount_rate', aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_differ_discount_rate_Coupon_cnt'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    keys = ['User_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data[data['is_manjian']==1], index=keys, values='cnt', aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
        'cnt': prefixs + 'receive_manjian_Coupon_cnt'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    keys = ['User_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data[data['is_manjian']==0], index=keys, values='cnt', aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
        'cnt': prefixs + 'receive_not_manjian_Coupon_cnt'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 用户是否在同一天重复领取了特定优惠卷
    print('-- 用户是否在同一天重复领取了特定优惠卷')
    keys = ['User_id', 'Coupon_id', 'Date_received']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    ...
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_discount_rate_mean'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_discount_rate_min'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_discount_rate_max'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 用户领取优惠卷的折扣率中位数
    print('-- 用户领取优惠卷的折扣率中位数')
    keys = ['User_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data, index=keys, values='discount_rate', aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_discount_rate_median'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    keys = ['User_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data, index=keys, values='discount_rate', aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_discount_rate_var'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.pivot_table(data, index=keys, values='Distance',
                           aggfunc=np.mean)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_Distance_mean'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.pivot_table(data, index=keys, values='Distance',
                           aggfunc=np.max)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_Distance_max'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.pivot_table(data, index=keys, values='Distance',
                           aggfunc=np.min)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_Distance_min'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.pivot_table(data, index=keys, values='Distance',
                           aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_Distance_median'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    pivot = pd.pivot_table(data, index=keys, values='Distance',
                           aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_Distance_var'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
...
def get_history_field_merchant_feature(label_field, history_field):
	...
	- 历史上商家优惠卷被领取后核销次数
	- 历史上商家优惠卷被领取后核销率
	- 历史上商家优惠卷核销的平均/最小/最大消费折率
	- 历史上商家优惠卷核销的消费折率的中位数/方差
	- 历史上商家提供的不同优惠卷数目
	- 历史上领取商家优惠卷的不同用户数量
	- 历史上核销商家优惠卷的不同用户数量，及其占领取不同的用户比重
	- 历史上商家优惠卷平均每个用户核销多少张
	- 历史上商家被核销过的不同优惠卷数量
	- 历史上商家平均每种优惠卷核销多少张
	- 历史上商家被核销过的不同优惠卷数量占所有领取过的不同优惠卷数量的比重
	- 历史上商家被核销优惠卷的平均时间间隔
	- 历史上商家被核销优惠卷中的平均/最小/最大用户-商家距离
    - 历史上商家被核销优惠卷中的用户-商家距离的中位数/方差
    - 历史上商家15天内被核销优惠卷的数量
    - 历史上商家15天内被核销优惠卷的种类数量
    - 历史上商家15天内被核销的不同种类优惠券占所有种类优惠券的比例
    - 历史上商家15天内优惠卷核销的平均/最小/最大消费折率
    - 历史上商家15天内优惠卷核销的消费折率的中位数/方差
    - 历史上商家15天内被核销优惠卷中的平均/最小/最大用户-商家距离
    - 历史上商家15天内被核销优惠卷中的用户-商家距离的中位数/方差
    - 历史上商家被多少个用户15天内核销优惠卷
    - 历史上商家15天内被核销优惠券的平均时间间隔
    ...
    # 历史上商家优惠卷核销的消费折率的中位数
    print('-- 历史上商家优惠卷核销的消费折率的中位数')
    pivot = pd.pivot_table(data, index=keys, values='discount_rate', aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_median_rate'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    
    # 历史上商家优惠卷核销的消费折率的方差
    print('-- 历史上商家优惠卷核销的消费折率的方差')
    pivot = pd.pivot_table(data, index=keys, values='discount_rate', aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_var_rate'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    # 历史上核销商家优惠卷的不同用户数量占领取不同的用户比重
    print('-- 历史上核销商家优惠卷的不同用户数量占领取不同的用户比重')
    ...
    # 历史上商家被核销过的不同优惠卷数量占所有领取过的不同优惠卷数量的比重
    print('-- 历史上商家被核销过的不同优惠卷数量占所有领取过的不同优惠卷数量的比重')
    ...
    # 历史上商家被核销优惠卷的平均时间间隔
    print('-- 历史上商家被核销优惠卷的平均时间间隔')
    tmp = data[data['date'].map(lambda x: str(x) != 'null')]
    tmp['gap'] = list(map(lambda x, y: (x - y).total_seconds() / (60 * 60 * 24), tmp['date'], tmp['date_received']))
    pivot = pd.pivot_table(tmp, index=keys, values='gap',
                           aggfunc=np.mean)
    pivot = pd.DataFrame(pivot).rename(columns={
        'gap': prefixs + 'receive_and_consume_time_gap_mean'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家被核销优惠卷中的平均用户 - 商家距离
    print('-- 历史上商家被核销优惠卷中的平均用户 - 商家距离')
    ...
    # 历史上商家被核销优惠卷中的最小用户 - 商家距离
    print('-- 历史上商家被核销优惠卷中的最小用户 - 商家距离')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.min)
    pivot = pd.DataFrame(pivot).rename(columns={
    ...
    # 历史上商家被核销优惠卷中的最大用户 - 商家距离
    print('-- 历史上商家被核销优惠卷中的最大用户 - 商家距离')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.max)
    pivot = pd.DataFrame(pivot).rename(columns={
    ...
    # 历史上商家被核销优惠卷中的用户 - 商家距离的中位数
    print('-- 历史上商家被核销优惠卷中的用户 - 商家距离的中位数')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max'}).reset_index()
    ...
    # 历史上商家被核销优惠卷中的用户 - 商家距离的方差
    print('-- 历史上商家被核销优惠卷中的用户 - 商家距离的方差')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max'}).reset_index()
    ...
    # 历史上商家15天内被核销优惠卷的数量
    print('-- 历史上商家15天内被核销优惠卷的数量')
    pivot = pd.pivot_table(data[data['label']==1],index=keys, values='cnt',
                           aggfunc=len)
    pivot = pd.DataFrame(pivot).rename(columns={
        'cnt': prefixs + 'receive_and_consume_cnt_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销优惠卷的种类数量
    print('-- 历史上商家15天内被核销优惠卷的数量')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='Coupon_id',
                           aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
        'Coupon_id': prefixs + 'receive_and_consume_differ_Coupon_cnt_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销的不同种类优惠券占所有种类优惠券的比例
    print('-- 历史上商家15天内被核销的不同种类优惠券占所有种类优惠券的比例')
    differ_Coupon = len(set(data['Coupon_id']))
    feature[prefixs + 'receive_and_consume_differ_Coupon_receive_differ_Coupon_rate_15'] = list(
        map(lambda x: x / differ_Coupon if differ_Coupon > 0 else 0,
            feature[prefixs + 'receive_and_consume_differ_Coupon_cnt_15']))
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内优惠卷核销的平均消费折率
    print('-- 历史上商家15天内优惠卷核销的平均消费折率')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='discount_rate', aggfunc=np.mean)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_mean_rate_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内优惠卷核销的最小消费折率
    print('-- 历史上商家15天内优惠卷核销的最小消费折率')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='discount_rate', aggfunc=np.min)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_min_rate_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内优惠卷核销的最大消费折率
    print('-- 历史上商家15天内优惠卷核销的最大消费折率')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='discount_rate', aggfunc=np.max)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_max_rate_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内优惠卷核销的消费折率的中位数
    print('-- 历史上商家15天内优惠卷核销的消费折率的中位数')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='discount_rate', aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_median_rate_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内优惠卷核销的消费折率的方差
    print('-- 历史上商家15天内优惠卷核销的消费折率的方差')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='discount_rate', aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'discount_rate': prefixs + 'receive_and_consume_var_rate_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销优惠卷中的平均用户 - 商家距离
    print('-- 历史上商家15天内被核销优惠卷中的平均用户 - 商家距离')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='Distance',
                           aggfunc=lambda x: sum([i for i in x if i >= 0]) / len(x) if len(x) > 0 else 0)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_mean_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销优惠卷中的最小用户 - 商家距离
    print('-- 历史上商家15天内被核销优惠卷中的最小用户 - 商家距离')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='Distance',
                           aggfunc=np.min)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_min_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销优惠卷中的最大用户 - 商家距离
    print('-- 历史上商家15天内被核销优惠卷中的最大用户 - 商家距离')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='Distance',
                           aggfunc=np.max)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销优惠卷中的用户 - 商家距离的中位数
    print('-- 历史上商家15天内被核销优惠卷中的用户 - 商家距离的中位数')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='Distance',
                           aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max_15'}).reset_index()
    ...
    # 历史上商家15天内被核销优惠卷中的用户 - 商家距离的方差
    print('-- 历史上商家15天内被核销优惠卷中的用户 - 商家距离的方差')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='Distance',
                           aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max_15'}).reset_index()
    ...
    # 历史上商家被多少个用户15天内核销优惠卷
    print('-- 历史上商家被多少个用户15天内核销优惠卷')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='User_id',
                           aggfunc=lambda x: len(set(x)))
    pivot = pd.DataFrame(pivot).rename(columns={
        'User_id': prefixs + 'receive_and_consume_differ_User_cnt_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)

    # 历史上商家15天内被核销优惠券的平均时间间隔
    print('-- 历史上商家15天内被核销优惠券的平均时间间隔')
...
def get_history_field_coupon_feature(label_field, history_field):
    ...
    # 历史上该优惠卷15天内被消费的次数
    print('-- 历史上该优惠卷15天内被消费的次数')
    pivot = pd.pivot_table(data[data['label'] == 1], index=keys, values='cnt',
                           aggfunc=len)
    pivot = pd.DataFrame(pivot).rename(
        columns={'cnt': prefixs + 'receive_and_consume_cnt_15'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    feature.fillna(0, downcast='infer', inplace=True)
    ...
    # 满减优惠卷最低消费的中位数
    print('-- 历史上满减优惠卷最低消费的中位数')
    # 先筛选出is_manjian为1即满减卷的样本,以keys为键,'min_cost_of_manjian'为值,使用np.median统计中位数
...
def get_history_field_user_feature(label_field, history_field, history_field_on):
	...
	- 历史上用户获得优惠卷并核销次数
	- 历史上用户领取优惠卷后进行核销率
    - 历史上用户核销优惠卷的平均/最低/最高消费折率
    - 历史上用户领取多少个不同的商家
    - 历史上用户领取不同的商家数量占所有不同商家的比重
	- 历史上用户核销过优惠卷的不同商家数量，及其占所有不同商家的比重
	- 历史上用户核销过优惠卷的不同商家数量占领取过的不同商家的数量的比重
	- 历史上用户核销过的不同优惠卷数量，及其占所有不同优惠卷的比重
	- 历史上用户对领卷商家的15天内的核销数
	- 历史上用户平均核销每个商家多少张优惠卷
	- 历史上用户核销优惠卷中的平均/最大/最小用户-商家距离
	- 历史上用户领取满0~50/50~200/200~500 减的优惠卷次数
    - 历史上用户核销满0~50/50~200/200~500 减的优惠卷次数
    - 历史上用户未核销满0~50/50~200/200~500 减的优惠卷次数
	- 历史上用户满0~50/50~200/200~500 减的优惠卷核销率
	- 历史上用户核销满0~50/50~200/200~500减的优惠卷占所有核销优惠卷的比重
    ...
    # 历史上用户核销过优惠卷的不同商家数量占所有不同商家的比重
    print('-- 历史上用户核销过优惠卷的不同商家数量占所有不同商家的比重')
    ...
    # 历史上用户核销过优惠卷的不同商家数量占领取过的不同商家的数量的比重
    print('-- 历史上用户核销过优惠卷的不同商家数量占领取过的不同商家的数量的比重')
    ...
    # 历史上用户核销优惠卷中的平均用户 - 商家距离
    print('-- 历史上用户核销优惠卷中的平均用户 - 商家距离')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=lambda x: sum([i for i in x if i >= 0]) / len(x) if len(x) > 0 else 0)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_mean'}).reset_index()
    ...
    # 历史上用户核销优惠卷中的最大用户 - 商家距离
    print('-- 历史上用户核销优惠卷中的最大用户 - 商家距离')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.max)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max'}).reset_index()
    ...
    # 历史上用户核销优惠卷中的最小用户 - 商家距离
    print('-- 历史上用户核销优惠卷中的最小用户 - 商家距离')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.min)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_min'}).reset_index()
    ...
    # # 历史上用户领取满0~50 / 50~200 / 200~500 减的优惠卷次数
    # print('-- 历史上用户领取满0~50 / 50~200 / 200~500 减的优惠卷次数')
    # keys = ['User_id']
    # prefixs = 'history_field_' + '_'.join(keys) + '_'
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_0_50'] == 1], index=keys,
    #                        values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_min_cost_of_manjian_0_50_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    #
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_50_200'] == 1],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_min_cost_of_manjian_50_200_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    #
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_200_500'] == 1],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_min_cost_of_manjian_200_500_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    # feature.fillna(0, downcast='infer', inplace=True)
    #
    # # 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠卷次数
    # print('-- 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠卷次数')
    # keys = ['User_id']
    # prefixs = 'history_field_' + '_'.join(keys) + '_'
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_0_50'] == 1 & data['Date'].map(
    #     lambda x: str(x) != 'null')],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_and_consume_min_cost_of_manjian_0_50_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    #
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_50_200'] == 1 & data['Date'].map(
    #     lambda x: str(x) != 'null')],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_and_consume_min_cost_of_manjian_50_200_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    #
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_200_500'] == 1 & data['Date'].map(
    #     lambda x: str(x) != 'null')],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_and_consume_min_cost_of_manjian_200_500_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    # feature.fillna(0, downcast='infer', inplace=True)
    #
    # # 历史上用户未核销满0~50 / 50~200 / 200~500 减的优惠卷次数
    # print('-- 历史上用户未核销满0~50 / 50~200 / 200~500 减的优惠卷次数')
    # keys = ['User_id']
    # prefixs = 'history_field_' + '_'.join(keys) + '_'
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_0_50'] == 1 & data['Date'].map(
    #     lambda x: str(x) == 'null')],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_and_unconsume_min_cost_of_manjian_0_50_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    #
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_50_200'] == 1 & data['Date'].map(
    #     lambda x: str(x) == 'null')],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_and_unconsume_min_cost_of_manjian_50_200_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    #
    # pivot = pd.pivot_table(data[data['min_cost_of_manjian_200_500'] == 1 & data['Date'].map(
    #     lambda x: str(x) == 'null')],
    #                        index=keys, values='cnt', aggfunc=len)
    # pivot = pd.DataFrame(pivot).rename(columns={
    #     'cnt': prefixs + 'receive_and_unconsume_min_cost_of_manjian_200_500_cnt'}).reset_index()
    # feature = pd.merge(feature, pivot, on=keys, how='left')
    # feature.fillna(0, downcast='infer', inplace=True)
    #
    # # 历史上用户满0~50 / 50~200 / 200~500 减的优惠卷核销率
    # print('-- 历史上用户满0~50 / 50~200 / 200~500 减的优惠卷核销率')
    # keys = ['User_id']
    # prefixs = 'history_field_' + '_'.join(keys) + '_'
    # feature[prefixs + 'receive_and_consume_min_cost_of_manjian_0_50_rate'] = list(
    #     map(lambda x, y: x / y if y > 0 else 0,
    #         feature[prefixs + 'receive_and_consume_min_cost_of_manjian_0_50_cnt'],
    #         feature[prefixs + 'receive_min_cost_of_manjian_0_50_cnt']))
    #
    # feature[prefixs + 'receive_and_consume_min_cost_of_manjian_50_200_rate'] = list(
    #     map(lambda x, y: x / y if y > 0 else 0,
    #         feature[prefixs + 'receive_and_consume_min_cost_of_manjian_50_200_cnt'],
    #         feature[prefixs + 'receive_min_cost_of_manjian_50_200_cnt']))
    #
    # feature[prefixs + 'receive_and_consume_min_cost_of_manjian_200_500_rate'] = list(
    #     map(lambda x, y: x / y if y > 0 else 0,
    #         feature[prefixs + 'receive_and_consume_min_cost_of_manjian_200_500_cnt'],
    #         feature[prefixs + 'receive_min_cost_of_manjian_200_500_cnt']))
    # feature.fillna(0, downcast='infer', inplace=True)
    #
    # # 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠卷占所有核销优惠卷的比重
    # print('-- 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠卷占所有核销优惠卷的比重')
    # receive_and_consume_len=len(data[data['Date'].map(lambda x: str(x) != 'null')]['cnt'])
    # keys = ['User_id']
    # prefixs = 'history_field_' + '_'.join(keys) + '_'
    # feature[prefixs + 'receive_and_consume_min_cost_of_manjian_0_50_rate'] = list(
    #     map(lambda x: x / receive_and_consume_len if receive_and_consume_len > 0 else 0,
    #         feature[prefixs + 'receive_and_consume_min_cost_of_manjian_0_50_cnt']))
    #
    # feature[prefixs + 'receive_and_consume_min_cost_of_manjian_50_200_rate'] = list(
    #     map(lambda x: x / receive_and_consume_len if receive_and_consume_len > 0 else 0,
...
def get_history_field_user_merchant_coupon_feature(history_feat):
	...
	- 历史上用户对每个商家的优惠卷核销次数占用户总的核销次数的比重
	- 历史上用户对每个商家的不核销次数占商家总的不核销次数的比重
	- 历史上用户对每个商家的优惠卷核销次数占商家总的核销次数的比重

    """
    print('- 用户-商家-优惠卷综合特征')
    data=history_feat.copy()
    # 将User_id列中float类型的元素转换为int类型,因为列中存在np.nan即空值会让整列的元素变为float
    data['User_id'] = data['User_id'].map(int)
    # 将Merchant_id列中float类型的元素转换为int类型,因为列中存在np.nan即空值会让整列的元素变为float
    data['Merchant_id'] = data['Merchant_id'].map(int)
    ...
    # 历史上用户对每个商家的优惠卷核销次数占用户总的核销次数的比重
    print('-- 历史上用户对每个商家的优惠卷核销次数占用户总的核销次数的比重')
    keys = ['User_id', 'Merchant_id']
    prefixs = 'history_field_' + '_'.join(keys) + '_'
    ...
    # 历史上用户对每个商家的优惠卷核销次数占商家总的核销次数的比重
    print('-- 历史上用户对每个商家的优惠卷核销次数占商家总的核销次数的比重')
    keys = ['User_id', 'Merchant_id']
    prefixs = 'history_field_' + '_'.join(keys) + '_'
...
def get_dataset(history_field, history_field_on, label_field):
    """构造数据集
    ...
    dataset = pd.concat(
        [history_feat, label_feat.drop(share_characters, axis=1)], axis=1, join_axes=[history_feat.index])
...
if __name__ == '__main__':
    ...
    # 源数据
    off_train = pd.read_csv(r'./input_files/ccf_offline_stage1_train.csv')#.sample(frac=0.01).reset_index(drop=True)
    on_train = pd.read_csv(r'./input_files/ccf_online_stage1_train.csv')#.sample(frac=0.01).reset_index(drop=True)
    off_test = pd.read_csv(r'./input_files/ccf_offline_stage1_test_revised.csv')#.sample(frac=0.01).reset_index(drop=True)
    # 预处理
    ...
    train_history_field = off_train[
        off_train['date_received'].isin(pd.date_range('2016/3/2', periods=75))]  # [20160302,20160501)
    train_history_field_on = on_train[on_train['act_date'].isin(pd.date_range('2016/3/2', periods=75))]
    ...
    validate_history_field = off_train[
        off_train['date_received'].isin(pd.date_range('2016/1/16', periods=75))]  # [20160116,20160316)
    validate_history_field_on = on_train[on_train['act_date'].isin(pd.date_range('2016/1/16', periods=75))]
    ...
    test_history_field = off_train[
        off_train['date_received'].isin(pd.date_range('2016/4/17', periods=75))]  # [20160417,20160616)
    test_history_field_on = on_train[on_train['act_date'].isin(pd.date_range('2016/4/17', periods=75))]
    test_label_field = off_test.copy()  # [20160701,20160801)

    # 构造训练集、验证集、测试集
    print('\n构造训练集')
    train = get_dataset(train_history_field, train_history_field_on, train_label_field)
    print('\n构造验证集')
    validate = get_dataset(validate_history_field, validate_history_field_on, validate_label_field)
    print('\n构造测试集')
    test = get_dataset(test_history_field, test_history_field_on, test_label_field)
