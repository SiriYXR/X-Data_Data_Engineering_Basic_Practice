...
def get_history_field_merchant_feature(label_field, history_field):
    ...
    # 历史上商家被核销优惠券中的用户 - 商家距离的中位数
    print('-- 历史上商家被核销优惠券中的用户 - 商家距离的中位数')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.median)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max'}).reset_index()
    ...
    # 历史上商家被核销优惠券中的用户 - 商家距离的方差
    print('-- 历史上商家被核销优惠券中的用户 - 商家距离的方差')
    pivot = pd.pivot_table(data[data['Date'].map(lambda x: str(x) != 'null')], index=keys, values='Distance',
                           aggfunc=np.var)
    pivot = pd.DataFrame(pivot).rename(columns={
        'Distance': prefixs + 'receive_and_consume_Distance_max'}).reset_index()
...
def get_history_field_user_feature(label_field, history_field, history_field_on):
	...
	- 历史上用户领取满0~50/50~200/200~500 减的优惠券次数
    - 历史上用户核销满0~50/50~200/200~500 减的优惠券次数
    - 历史上用户未核销满0~50/50~200/200~500 减的优惠券次数
    ...
    # 历史上用户领取满0~50 / 50~200 / 200~500 减的优惠券次数
    print('-- 历史上用户领取满0~50 / 50~200 / 200~500 减的优惠券次数')
    keys = ['User_id']
    prefixs = 'history_field_' + '_'.join(keys) + '_'
    ...
    # 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠券次数
    print('-- 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠券次数')
    keys = ['User_id']
    prefixs = 'history_field_' + '_'.join(keys) + '_'
    ...
    # 历史上用户未核销满0~50 / 50~200 / 200~500 减的优惠券次数
    print('-- 历史上用户未核销满0~50 / 50~200 / 200~500 减的优惠券次数')
    keys = ['User_id']
    prefixs = 'history_field_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data[data['min_cost_of_manjian_0_50'] == 1 & data['Date'].map(
        lambda x: str(x) == 'null')],
    ...
    # 历史上用户满0~50 / 50~200 / 200~500 减的优惠券核销率
    print('-- 历史上用户满0~50 / 50~200 / 200~500 减的优惠券核销率')
    keys = ['User_id']
    prefixs = 'history_field_' + '_'.join(keys) + '_'
    ...
    print('-- 历史上用户核销满0~50 / 50~200 / 200~500 减的优惠券占所有核销优惠券的比重')
