...
if __name__ == '__main__':
    ...
    # 源数据
    off_train = pd.read_csv(r'./input/ccf_offline_stage1_train.csv')
    on_train = pd.read_csv(r'./input/ccf_online_stage1_train.csv')
    off_test = pd.read_csv(r'./input/ccf_offline_stage1_test_revised.csv')
    ...
    # validate.to_csv("./dataset/validate.csv", index=False)
    # test.to_csv("./dataset/test.csv", index=False)
    train = pd.read_csv(r'./dataset/train.csv')
    validate = pd.read_csv("./dataset/validate.csv")
    test = pd.read_csv("./dataset/test.csv")

    # 训练
    result_lgbm,_ = model_lgbm(train, validate, test)
    result_gbdt,_ = model_gbdt(train, validate, test)

    result_merge = result_gbdt.copy()
    result_merge.iloc[:, 3] = result_gbdt.iloc[:, 3] * (2 / 10.0) + result_lgbm.iloc[:, 3] * (1 - (2 / 10.0))

    result_lgbm.to_csv(r'./output/result_lgbm.csv', index=False, header=None)
    result_gbdt.to_csv(r'./output/result_gbdt.csv', index=False, header=None)
    result_merge.to_csv(r'./output/result_merge.csv', index=False, header=None)
